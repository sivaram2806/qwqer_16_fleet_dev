...
class BalanceSheetView(models.TransientModel):
    ...
    def view_report(self, option, tag, lang):
        ...
        for rec in records['Accounts']:
            ...
            move_lines_dict[rec['id']]['init_debit'] = rec['init_debit']
            move_lines_dict[rec['id']]['init_credit'] = rec['init_credit']
            move_lines_dict[rec['id']]['init_balance'] = rec['init_balance']
        report_lines_move = []
        parent_list = []
        ...
        def filter_sum(obj):
            ...
            for pl in parent_list:
                ...
                sum_list[pl]['s_init_debit'] = 0
                sum_list[pl]['s_init_credit'] = 0
                sum_list[pl]['s_init_balance'] = 0
            ...
            for each in obj:
                if each['p_id'] and each['p_id'] in parent_list:
                    ...
                    sum_list[each['p_id']]['s_init_debit'] += each['init_debit']
                    sum_list[each['p_id']]['s_init_credit'] += each['init_credit']
                    sum_list[each['p_id']]['s_init_balance'] += each['init_balance']
            return sum_list
        ...
        def assign_sum(obj):
            for each in obj:
                if each['r_id'] in parent_list and \
                    ...
                    each['init_debit'] = sum_list_new[each['r_id']]['s_init_debit']
                    each['init_credit'] = sum_list_new[each['r_id']]['s_init_credit']
        ...
        for rec in final_report_lines:
            ...
            if position == "before":
                ...
                rec['m_balance'] = "{:,.2f}".format(
                    rec['balance']) + " " + symbol
            rec['init_debit'] = round(rec.get('init_debit', 0.0), 2)
            rec['init_credit'] = round(rec.get('init_credit', 0.0), 2)
            rec['init_balance'] = rec.get('init_debit', 0.0) - rec.get('init_credit', 0.0)
            rec['init_balance'] = round(rec['init_balance'], 2)
            if (rec['init_balance_cmp'] < 0 and rec['init_balance'] > 0) or (
                    rec['init_balance_cmp'] > 0 and rec['init_balance'] < 0):
                rec['init_balance'] = rec['init_balance'] * -1

            if position == "before":
                rec['m_init_debit'] = symbol + " " + "{:,.2f}".format(rec['init_debit'])
                rec['m_init_credit'] = symbol + " " + "{:,.2f}".format(rec['init_credit'])
                rec['m_init_balance'] = symbol + " " + "{:,.2f}".format(rec['init_balance'])
            else:
                rec['m_init_debit'] = "{:,.2f}".format(rec['init_debit']) + " " + symbol
                rec['m_init_credit'] = "{:,.2f}".format(rec['init_credit']) + " " + symbol
                rec['m_init_balance'] = "{:,.2f}".format(rec['init_balance']) + " " + symbol
        user = self.env.user
        ...
        for line in records['Accounts']:
            ...
            if account_id not in merged_data:
                ...
                merged_data[account_id]['init_debit'] += line['init_debit']
                merged_data[account_id]['init_credit'] += line['init_credit']
                merged_data[account_id]['init_balance'] += line['init_balance']
    ...
    def _get_report_values(self, data):
        ...
        for rec in account_res:
            ...
            # if data.get('date_from'):
            #     rec['Init_balance'] = self.get_init_bal(rec['account_id'], display_account, data)
    ...
    def _get_accounts(self, accounts, init_balance, display_account, data):
        ...
        combined_sql = ('''
            SELECT account_id, 
                   ...
                   id, 
                   name, 
                   SUM(init_debit) AS init_debit, 
                   SUM(init_credit) AS init_credit, 
                   SUM(init_balance) AS init_balance, 
                   SUM(debit) AS debit, 
                   ...
                   tag 
            FROM (
                -- First Query (init columns set to zero)
                SELECT l.account_id AS account_id, 
                       a.code AS code, 
                       a.id AS id, 
                       a.name AS name, 
                       0 AS init_debit,  -- Set init_debit to 0
                       0 AS init_credit,  -- Set init_credit to 0
                       0 AS init_balance,  -- Set init_balance to 0
                ...
                LEFT JOIN account_account_account_tag acct ON (acct.account_account_id = l.account_id) 
                LEFT JOIN account_account_tag act ON (act.id = acct.account_account_tag_id) 
                LEFT JOIN LATERAL ( 
                ...
                GROUP BY l.account_id, a.code, a.id, a.name, anl.keys, act.name

                UNION ALL

                -- Second Query (calculating init values)
                SELECT l.account_id AS account_id, 
                       a.code AS code, 
                       a.id AS id, 
                       a.name AS name, 
                       ROUND(COALESCE(SUM(l.debit), 0), 2) AS init_debit, 
                       ROUND(COALESCE(SUM(l.credit), 0), 2) AS init_credit, 
                       ROUND(COALESCE(SUM(l.balance), 0), 2) AS init_balance,
                       0 AS debit,  -- Set debit to 0
                       0 AS credit,  -- Set credit to 0
                       0 AS balance,  -- Set balance to 0
                       anl.keys, 
                       act.name AS tag 
                FROM account_move_line l 
                JOIN account_move m ON (l.move_id = m.id) 
                ...
                LEFT JOIN account_analytic_account an ON (anl.keys = an.id) 
                ''' + WHERE + old_final_filter + ''' 
                GROUP BY l.account_id, a.code, a.id, a.name, anl.keys, act.name
            ) AS combined_results 
            GROUP BY account_id, code, id, name, keys, tag
        ''')

        if data.get('accounts'):
            params = tuple(where_params+where_params)
        else:
            params = (tuple(accounts.ids),) + tuple(where_params)
        cr.execute(combined_sql, params)

        account_res = cr.dictfetchall()
        # initial_balance = self.get_init_bal(WHERE, where_params, old_final_filter, accounts, data)
        # result = self.merge_init_and_presnt_balance(account_res, initial_balance)
        return account_res
    ...
    def get_dynamic_xlsx_report(self, options, response, report_data,
        ...
        sheet.write(row, col + 4, 'Debit', sub_heading)
        sheet.write(row, col + 5, 'Credit', sub_heading)
        sheet.write(row, col + 6, 'Balance', sub_heading)
        ...
        if rl_data:
            for fr in rl_data:
                ...
                sheet.write(row, col + 1, fr['init_debit'], txt)
                sheet.write(row, col + 2, fr['init_credit'], txt)
                sheet.write(row, col + 3, fr['init_balance'], txt)
                sheet.write(row, col + 4, fr['debit'], txt)
                sheet.write(row, col + 5, fr['credit'], txt)
                sheet.write(row, col + 6, fr['balance'], txt)

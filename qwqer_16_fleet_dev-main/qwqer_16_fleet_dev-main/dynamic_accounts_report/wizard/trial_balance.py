...
class TrialView(models.TransientModel):
    ...
    def _get_accounts(self, accounts, display_account, data):
        ...
        for account in accounts:
            ...
            if account.id in account_result:
                ...
                balance = account_result[account.id].get('debit') - account_result[account.id].get('credit')
                res['end_debit'] = abs(round(balance, 2)) if balance > 0 else 0.0
                res['end_credit'] = abs(round(balance, 2)) if balance < 0 else 0.0
    ...
    def get_dynamic_xlsx_report(self, data, response ,report_data, dfr_data):
        ...
        if filters.get('date_from'):
            sheet.merge_range('C7:D7', "Initial", sub_heading)
            sheet.write('C8', 'Debit', sub_heading)
            sheet.write('D8', 'Credit', sub_heading)
            sheet.merge_range('E7:F7', "Period Transactions", sub_heading)
            ...
            sheet.merge_range('C7:D7', "Period Transactions", sub_heading)
            sheet.write('C8', 'Debit', sub_heading)
            sheet.write('D8', 'Credit', sub_heading)
            sheet.merge_range('E7:F7', "Ending Balance", sub_heading)
            sheet.write('E8', 'Debit', sub_heading)
            sheet.write('F8', 'Credit', sub_heading)
        ...
        if filters.get('date_from'):
            ...
            sheet.set_column(8, 3, 15)
            sheet.set_column(9, 4, 15)
        init_balance_debit = 0.0
        init_balance_credit = 0.0
        end_balance_debit = 0.0
        end_balance_credit = 0.0
        ...
        for rec_data in report_data_main:
            ...
            debit_credit = rec_data['debit'] - rec_data['credit']
            if filters.get('date_from'):
                if rec_data.get('Init_balance'):
                    ...
                    init_balance_debit += rec_data['Init_balance']['debit']
                    init_balance_credit += rec_data['Init_balance']['credit']
                else:
                ...
                sheet.write(row, col + 6, round(debit_credit, 2) if debit_credit > 0 else 0 , txt)
                sheet.write(row, col + 7, abs(round(debit_credit, 2)) if debit_credit < 0 else 0, txt)


            else:
                sheet.write(row, col + 2, rec_data['debit'], txt)
                sheet.write(row, col + 3, rec_data['credit'], txt)
                sheet.write(row, col + 4, round(debit_credit, 2) if debit_credit > 0 else 0 , txt)
                sheet.write(row, col + 5, abs(round(debit_credit, 2)) if debit_credit < 0 else 0, txt)
            if debit_credit> 0:
                end_balance_debit += round(debit_credit, 2)
            else:
                end_balance_credit += abs(round(debit_credit, 2))
        sheet.write(row+1, col, 'Total', sub_heading)
        if filters.get('date_from'):
            sheet.write(row + 1, col + 2, init_balance_debit, txt_l)
            sheet.write(row + 1, col + 3, init_balance_credit, txt_l)
            sheet.write(row + 1, col + 4, total.get('debit_total'), txt_l)
            sheet.write(row + 1, col + 5, total.get('credit_total'), txt_l)
            sheet.write(row + 1, col + 6, end_balance_debit, txt_l)
            sheet.write(row + 1, col + 7, end_balance_credit, txt_l)
        else:
            sheet.write(row + 1, col + 2, total.get('debit_total'), txt_l)
            sheet.write(row + 1, col + 3, total.get('credit_total'), txt_l)
            sheet.write(row + 1, col + 4, end_balance_debit, txt_l)
            sheet.write(row + 1, col + 5, end_balance_credit, txt_l)

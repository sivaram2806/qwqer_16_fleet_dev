...
class BalanceSheet(models.TransientModel):
    ...
    def _compute_account_balance(self, accounts):
        ...
        if accounts:
            ...
            for row in self.env.cr.dictfetchall():
                res[row['id']] = row
            if len(where_params) == 8:
                where_params.pop(1)
            if len(where_params) >= 7:
                new_request = request.replace("""("account_move_line"."date" >= %s)""", """("account_move_line"."date" < %s)""" ).replace("""("account_move_line"."date" <= %s) AND""", """""")
                where_params
                params = (tuple(accounts._ids),) + tuple(where_params)

                self.env.cr.execute(new_request, params)
                for row in self.env.cr.dictfetchall():
                    if res.get(row['id']):
                        res[row['id']].update({"init_debit": row["debit"],
                                               "init_credit": row["credit"],
        ...
        for key, vals in res.items():
            if "init_debit" not in vals:
                vals.update({"init_debit": 0,
                                               "init_credit": 0,
                                               "init_balance": 0})
        return res
    ...
    def _compute_report_balance(self, reports):
        ...
        res = {}
        fields = ['credit', 'debit', 'balance', 'init_credit', 'init_debit', 'init_balance']
    ...
    def get_account_lines(self, data):
        ...
        if data['enable_filter']:
            ...
            for report_id, value in comparison_res.items():
                ...
                res[report_id]['init_comp_bal'] = value['init_balance']
                report_acc = res[report_id].get('account')
                if report_acc:
                    for account_id, val in comparison_res[report_id
                    ].get('account').items():
                        report_acc[account_id]['comp_bal'] = val['balance']
                        report_acc[account_id]['init_comp_bal'] = val['init_balance']
        for report in child_reports:
            ...
            vals = {
                ...
                'init_balance': res[report.id].get('init_balance', 0.0) or 0.0,
                'type': 'report',
                'level': bool(
            ...
            if data['debit_credit']:
                ...
                vals['init_debit'] = res[report.id].get('init_debit', 0.0)
                vals['init_credit'] = res[report.id].get('init_credit', 0.0)

            if data['enable_filter']:
                vals['balance_cmp'] = res[report.id]['comp_bal'] * int(
                    report.sign)
                vals['init_balance_cmp'] = res[report.id]['init_comp_bal'] * int(
                    report.sign)
            lines.append(vals)
            ...
            if res[report.id].get('account'):
                ...
                for account_id, value \
                    ...
                    vals = {
                        ...
                        'init_balance': value.get('init_balance', 0.0) or 0.0,
                        'type': 'account',
                    ...
                    if data['debit_credit']:
                        ...
                        vals['init_debit'] = value.get('init_debit', 0.0)
                        vals['init_credit'] = value.get('init_credit', 0.0)
                    ...
                    if data['enable_filter']:
                        vals['balance_cmp'] = value['comp_bal'] * int(
                            report.sign)
                        vals['init_balance_cmp'] = value['init_comp_bal'] * int(
                            report.sign)
                        if not account.company_id.currency_id.is_zero(
                                vals['balance_cmp']):
                            flag = True

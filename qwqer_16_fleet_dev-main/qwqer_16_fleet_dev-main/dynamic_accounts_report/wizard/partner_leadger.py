...
class PartnerView(models.TransientModel):
    ...
    def _get_partners(self, partners, accounts, init_balance, display_account,
        ...
        sql = ('''SELECT l.id AS lid,l.partner_id AS partner_id,
                ...
                m.name AS move_name,m.ref AS reference, c.symbol AS currency_code,c.position
        ...
        for partner in partners:
            ...
            for line in res.get('move_lines'):
                ...
                if data.get('date_from'):
                    ...
                    opening_where_condition += """ AND (aml.partner_id = {0})""".format(partner.id)
                    if data['target_move'] == 'posted':
                        opening_where_condition += """ AND am.state = 'posted'"""
                    account_ids = data.get('accounts')
                    account_filter = data.get('account_type')
                    if account_ids and not account_filter:
                        ...
                        if account_filter == 'asset_receivable':
                            opening_where_condition += """ AND aa.account_type = 'asset_receivable'"""

                        elif account_filter == 'liability_payable':
                            opening_where_condition += """ AND aa.account_type = 'liability_payable'"""

                        else:
                            opening_where_condition += """ AND aa.account_type In ('asset_receivable','liability_payable')"""
                    cr.execute("""
                            Select
                    ...
                    opening_balance = dict_val and dict_val.get('balance') is not None and dict_val.get(
                        'balance') or False
                    res['opening_balance'] = round(opening_balance, 2)
            else:
                res['opening_balance'] = 0.00
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        sheet.merge_range('A1:F2',
                          filters.get('company_name'),
                          head)
        sheet.merge_range('A3:F5',
                          data,
                          sub_head)
        date_head = workbook.add_format({'align': 'center', 'bold': True,
                                         'font_size': '10px'})

        sheet.merge_range('A7:B7',
                          'Target Moves: ' + filters.get('target_move'),
                          date_head)

        sheet.merge_range('C7:D7', 'Account Type: ' +
                          filters['account_type'],
                          date_head)

        sheet.merge_range('A6:B6', ' Journals: ' + ', '.join(
            [lt or '' for lt in
             filters['journals']]),
                          date_head)
        sheet.merge_range('C6:D6', ' Accounts: ' + ', '.join(
            [lt or '' for lt in
             filters['accounts']]),
                          date_head)

        if filters.get('date_from') and filters.get('date_to'):
            sheet.merge_range('E7:F7', 'From: ' + filters.get('date_from'),
                              date_head)

            sheet.merge_range('E8:F8', 'To: ' + filters.get('date_to'),
                              date_head)
        elif filters.get('date_from'):
            sheet.merge_range('E7:F7', 'From: ' + filters.get('date_from'),
                              date_head)
        elif filters.get('date_to'):
            sheet.merge_range('E7:F7', 'To: ' + filters.get('date_to'),
                              date_head)

        sheet.merge_range('A8:B8', ' Partner Tags: ' + ', '.join(
            [lt or '' for lt in
             filters['partner_tags']]),
                          date_head)
        sheet.merge_range('A9:B9', ' Partners: ' + ', '.join(
            [lt or '' for lt in
             filters['partners']]), date_head)

        sheet.merge_range('A10:B10', 'Partner', cell_format)
        sheet.write('C10', 'Opening Balance', cell_format)
        sheet.write('E10', 'Debit', cell_format)
        sheet.write('D10', 'Credit', cell_format)
        sheet.write('F10', 'Balance', cell_format)
        row = 9
        col = 0
        ...
        for report in report_data:
            ...
            row += 1
            sheet.merge_range(row, col + 0, row, col + 1, report['name'],
                              sub_heading_sub)
            sheet.write(row, col + 2, report['opening_balance'], sub_heading_sub)
            sheet.write(row, col + 3, report['debit'], sub_heading_sub)
            sheet.write(row, col + 4, report['credit'], sub_heading_sub)
            sheet.write(row, col + 5, report['balance'], sub_heading_sub)
            row += 1
            sheet.write(row, col + 0, 'Date', cell_format)
            # sheet.write(row, col + 1, 'JRNL', cell_format)
            # sheet.write(row, col + 2, 'Account', cell_format)
            sheet.write(row, col + 1, 'Reference', cell_format)
            sheet.write(row, col + 2, 'Description', cell_format)
            sheet.write(row, col + 3, 'Debit', cell_format)
            sheet.write(row, col + 4, 'Credit', cell_format)
            sheet.write(row, col + 5, 'Balance', cell_format)
            for r_rec in report['move_lines']:
                ...
                sheet.write(row, col + 1, r_rec['lname'], txt)
                sheet.write(row, col + 2, r_rec['reference'], txt)
                sheet.write(row, col + 3, r_rec['debit'], txt)
                sheet.write(row, col + 4, r_rec['credit'], txt)
                sheet.write(row, col + 5, r_rec['balance'], txt)

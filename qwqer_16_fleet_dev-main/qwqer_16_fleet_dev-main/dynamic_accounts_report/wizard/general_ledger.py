...
class GeneralView(models.TransientModel):
    ...
    def get_accounts_line(self, account_id, title):
        ...
        # move lines
        sql = ('''SELECT * FROM (SELECT DISTINCT ON (l.id) l.id AS lid,m.id AS move_id, l.account_id AS account_id,
                ...
                p.name AS partner_name, anl.keys, m.ref AS move_ref
                FROM account_move_line l
               ...
               l.ref, l.name, m.name, c.symbol, c.position, p.name, anl.keys) t ORDER BY t.ldate''')
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        txt_new = workbook.add_format({'font_size': '10px', 'border': 1,'bg_color': '#ff9999'})
        ...
        txt_two_decimals = workbook.add_format({'font_size': '10px', 'border': 1, 'num_format': '0.00'})
        ...
        sheet.write('G8', 'Ref', sub_heading)
        ...
        sheet.write('L8', 'Running Balance', sub_heading)
        row = 6
        col = 0
        ...
        for rec_data in report_data_main:
            language = user_language if user_language in rec_data[
                'name'] else 'en_US'
            row += 1
            sheet.write(row + 1, col, rec_data['code'], txt_new)
            if isinstance(rec_data['name'], dict):
                sheet.write(row + 1, col + 1, rec_data['name'][language], txt_new)
            else:
                sheet.write(row + 1, col + 1, rec_data['name'], txt_new)
            sheet.write(row + 1, col + 2, '', txt_new)
            sheet.write(row + 1, col + 3, '', txt_new)
            sheet.write(row + 1, col + 4, '', txt_new)
            sheet.write(row + 1, col + 5, '', txt_new)
            sheet.write(row + 1, col + 6, '', txt_new)
            sheet.write(row + 1, col + 7, '', txt_new)
            sheet.write(row + 1, col + 8, rec_data['debit'], txt_new)
            sheet.write(row + 1, col + 9, rec_data['credit'], txt_new)
            sheet.write(row + 1, col + 10, rec_data['balance'], txt_new)
            sheet.write(row + 1, col + 11, rec_data['balance'], txt_new)
            running_balance = 0
            for line_data in rec_data['move_lines']:
                row += 1
                ldate_str = line_data.get('ldate')
                # Reformat the date string from yyyy-mm-dd to dd-mm-yyyy
                if ldate_str:
                    try:
                        formatted_date = datetime.strptime(ldate_str, "%Y-%m-%d").strftime("%d-%m-%Y")
                        sheet.write(row + 1, col + 2, formatted_date, txt)
                    except ValueError:
                ...
                sheet.write(row + 1, col + 6, line_data.get('move_ref'), txt)
                ...
                running_balance += line_data.get('debit', 0) - line_data.get('credit', 0)
                sheet.write(row + 1, col + 11, round(running_balance, 2), txt_two_decimals)  # Applying two-decimal format

...
class DeliveryMerchantPayoutLines(models.Model):
    _name = 'qshop.merchant.payout.lines'
    _description = 'Payout Lines'
    _rec_name = 'customer_id'

    @api.model
    def _default_qshop_service_type(self):
        service_type = self.env['partner.service.type'].search(
            [('is_qshop_service', '=', True), ('company_id', '=', self.env.company.id)], limit=1)
    ...
    line_ids = fields.Many2many(comodel_name='account.move.line', relation='qshop_move_payout_lines_rel',
    ...
    payout_id = fields.Many2one(comodel_name='qshop.merchant.payout', string="Payout")
    ...
    total_pay = fields.Float(string='Total Amount(A)', digits='Product Price', compute='compute_total_amount',
                             store=True)
    balance_amt = fields.Float(string='Deduction(D)', digits='Product Price')
    service_charge = fields.Float(string='Service Charge(E)', digits='Product Price', store=True)
    taxes = fields.Float(string='Taxes(F)', digits='Product Price', store=True)
    final_pay = fields.Float(string='Total Payout (A-B-C-D-E-F)', compute='get_total_payout', store=True,
    ...
    tds = fields.Float(string='TDS(B)', digits='Product Price', store=True)
    tcs = fields.Float(string='TCS(C)', digits='Product Price', store=True)
    tcs_tax_id = fields.Many2one("account.tax", string="TCS Tax", compute='_get_tax', store=True)
    tds_tax_id = fields.Many2one("account.tax", string="TDS Tax", compute='_get_tax', store=True)
    status_description = fields.Text(string="Status Description")
    qshop_service_type_id = fields.Many2one(comodel_name='partner.service.type',
                                               default=lambda self: self._default_qshop_service_type())

    @api.depends("customer_id")
    def _get_tax(self):
        for rec in self:
            rec.write({'tcs_tax_id': False, 'tds_tax_id': False})
            state_journal = self.env['state.journal'].search(
                [('state_id', '=', rec.payout_id.region_id.state_id.id), ('qshop_tcs_tax_id', '!=', False)], limit=1)
            if state_journal:
                rec.tcs_tax_id = state_journal.qshop_tcs_tax_id.id
            accounts_data = self.env['merchant.payout.account.config'].sudo().search(
                [('qshop_tds_tax_id', '!=', False)], limit=1)
            if accounts_data:
                rec.tds_tax_id = accounts_data.qshop_tds_tax_id.id

    @api.model
    def create(self, vals):
        company_id = self.env.company.id
        sequence = self.env['ir.sequence'].search(
            [('company_id', '=', company_id), ('code', '=', 'qshop.merchant.payout.line.seq')])
        if not sequence:
            sequence = self.env['ir.sequence'].search([('code', '=', 'qshop.merchant.payout.line.seq')], limit=1)
            new_sequence = sequence.sudo().copy()
            new_sequence.company_id = company_id
            vals['transfer_ref'] = new_sequence.next_by_id()
        else:
            vals['transfer_ref'] = self.env['ir.sequence'].next_by_code('qshop.merchant.payout.line.seq')
        return super(DeliveryMerchantPayoutLines, self).create(vals)

    def unlink(self):
        for rec in self:
            if rec.line_ids:
                rec.line_ids.write({'qshop_merchant_payout_id': False})
        ...
        for rec in self:
            if rec.line_ids:
                res = self.env['account.move.line'].sudo().search([('partner_id', '=', rec.customer_id.id), (
                    'qshop_merchant_payout_id', '=', rec._origin.payout_id.id)]).ids
                if res:
                    res1 = list(set(rec.line_ids.ids).symmetric_difference(set(res)))
                    if res1:
                        self.env['account.move.line'].sudo().browse(res1).write(({
                            'qshop_merchant_payout_id': False
                        }))

    @api.depends('line_ids', 'tds_tax_id', 'tcs_tax_id')
    def compute_total_amount(self):
        merchant_config = self.env['merchant.payout.account.config'].sudo().search([], limit=1)
        state_journal = self.env['state.journal'].search(
            [('state_id', '=', self.payout_id.region_id.state_id.id), ('qshop_tcs_tax_id', '!=', False)], limit=1)
        for rec in self:
            rec.total_pay = sum(rec.line_ids.mapped('credit')) or 0.00

            tds_tax = merchant_config.qshop_tds_tax_id or False
            if rec.tds_tax_id:
                tds_tax = rec.tds_tax_id
            tds = tds_tax and tds_tax.with_context(round=True).compute_all(rec.total_pay)
            tds_amt = 0
            tds_list = tds and tds.get('taxes', []) or []
            for data in tds_list:
                tds_amt += round(data.get('amount', 0.0), 2)
            rec.tds = tds_amt

            tcs_amt = 0
            if rec.customer_id.vat:
                tcs_tax = state_journal.qshop_tcs_tax_id or False
                if rec.tcs_tax_id:
                    tcs_tax = rec.tcs_tax_id
                tcs = tcs_tax and tcs_tax.with_context(round=True).compute_all(rec.total_pay)
                tcs_list = tcs and tcs.get('taxes', []) or []
                for data in tcs_list:
                    tcs_amt += round(data.get('amount', 0.0), 2)
            rec.tcs = tcs_amt
    ...
    def onchange_taxes(self):
        for rec in self:
            tax_amt = 0
            for ele in rec.customer_id.qshop_invoice_tax_ids:
    ...
    def get_total_payout(self):
        for rec in self:
            rec.final_pay = rec.total_pay - rec.tds - rec.tcs - rec.balance_amt - rec.service_charge - rec.taxes
    ...
    def unrec_entries(self):
        for rec in self:
            ...
            for line in rec.line_ids:
                line.qshop_merchant_payout_id = False

    def action_shop_xls_print(self):
        for rec in self:
            data = {
                'ids': self.env.context.get('active_ids', []),
                'model': 'qshop.payout.lines',
            }
            return self.env.ref(
                'qshop_base.qshop_merchant_payout_line_form_view_xlsx_report').report_action(self,data=data)
    ...
    def mail_sent_template(self):
        ...
        try:
            report_id = self.env.ref('qshop_base.qshop_merchant_payout_line_form_view_xlsx_report')
        ...
        mail_template_id = self.env.ref('qshop_base.mail_template_qshop_merchant_payout')
        ctx = {
            'default_model': 'qshop.merchant.payout.lines',
...
class SendMailExtended(models.TransientModel):
    ...
    def _action_send_mail(self, auto_commit=False):
        """
        Sends mail and updates the `is_mail_sent` flag for Qshop merchant payout.
        If an error occurs during mail sending, a `ValidationError` is raised.
        """
        active_model = self.env.context.get('active_model')
        if not active_model or active_model != 'qshop.merchant.payout.lines':

...
class MerchantPayout(models.Model):
    """This model qshop.merchant.payout is to make merchant payout  for qshop(qwqer shop ) merchant"""
    _name = 'qshop.merchant.payout'
    _description = 'Qshop Merchant Payout'
    ...
    line_ids = fields.One2many(comodel_name='qshop.merchant.payout.lines', inverse_name='payout_id',
    ...
    def create(self, vals):
        ...
        sequence = self.env['ir.sequence'].search(
            [('company_id', '=', company_id), ('code', '=', 'qshop.merchant.payout.seq')])
        if not sequence:
            sequence = self.env['ir.sequence'].search([('code', '=', 'qshop.merchant.payout.seq')], limit=1)
            new_sequence = sequence.sudo().copy()
            new_sequence.company_id = company_id
            vals['rec_name'] = new_sequence.next_by_id()
        else:
            vals['rec_name'] = self.env['ir.sequence'].next_by_code('qshop.merchant.payout.seq')
                    ...
                    for line in rec.line_ids:
                        if line.line_ids:
                            if len(line.line_ids) > 1:
                                query1 = (
                                        """UPDATE account_move_line SET qshop_merchant_payout_id = Null WHERE id in %s;""" % str(
                                    tuple(line.line_ids.ids)))
                            else:
                                query1 = (
                                        """UPDATE account_move_line SET qshop_merchant_payout_id = Null WHERE id = %d;""" % line.line_ids.id)
                ...
                if so_data:
                    for partner_id, order_ids in so_data.items():
                        query = self.env['account.move.line']._where_calc([
                            ...
                            ('service_type_id.is_qshop_service', '=', True)
                        ])
                        ...
                        if move_lines:
                            payout_line_id = rec.env['qshop.merchant.payout.lines'].sudo().create(
                                {'to_date': rec.to_date,
                                 'from_date': rec.from_date,
                                 'customer_id': partner_id,
                                 'payout_id': rec.id,
                                 'line_ids': move_lines})
                            payout_line_id.line_ids.write(({'qshop_merchant_payout_id': rec.id, }))
        ...
        for rec in self:
            if self.to_date and self.region_id:
                ...
                merchant_query = self.env['sale.order']._where_calc([
                    ...
                    ('partner_id.service_type_id.is_qshop_service', '=', True),
                    ('service_type_id.is_qshop_service', '=', True)
                ])
            ...
            if rec.state == 'draft':
                for line in rec.line_ids:
                    ...
                    if so_data:
                        query = self.env['account.move.line']._where_calc([
                            ('full_reconcile_id', '=', False),
                            ('qshop_merchant_payout_id', '=', rec.id),
                            ('balance', '!=', 0),
                            ('account_id.reconcile', '=', True),
    ...
    def action_batch_payment(self):
        for rec in self:
            try:
                query = "SELECT id FROM qshop_merchant_payout WHERE id = %s FOR UPDATE NOWAIT"
    ...
    def create_trns_journal_entry(self):
        for rec in self:
            qshop_service_type = self.env['partner.service.type'].search(
                [('is_qshop_service', '=', True), ("company_id", "=", self.company_id.id)], limit=1)
            ...
            if rec.line_ids and accounts_data:
                for line in rec.line_ids:
                    ...
                    if line.payment_state == 'success' and not line.payment_journal_id:
                        ...
                        if accounts_data.qshop_debit_transfer_account_id:
                            if line.balance_amt > 0:
                                move_line_2 = {'account_id': accounts_data.qshop_debit_transfer_account_id.id or False,
                                               'debit': 0.0,
                                               'credit': line.balance_amt,
                                               'analytic_distribution': {
                                                                            rec.region_id.analytic_account_id.id: 100} or False,
                            ...
                            if line.taxes + line.service_charge > 0:
                                move_line_3 = {'account_id': accounts_data.qshop_debit_transfer_account_id.id or False,
                                               'debit': 0.0,
                        ...
                        if accounts_data.qshop_tds_tax_payble:
                            if line.tds > 0:
                                move_line_4 = {'account_id': accounts_data.qshop_credit_transfer_account_id.id or False,
                                               'debit': 0.0,
                                               'credit': line.tds,
                                               'analytic_distribution': {
                                                                            rec.region_id.analytic_account_id.id: 100} or False,
                                               'name': 'Merchant Amount-TDS',
                                               'partner_id': line.customer_id.id or False,
                                               'service_type_id': qshop_service_type.id,
                                               }

                                move_line_list.append((0, 0, move_line_4))

                        if line.tcs > 0:
                            state_journal = self.env['state.journal'].search(
                                [('state_id', '=', rec.region_id.state_id.id), ('qshop_tcs_tax_id', '!=', False)],
                                limit=1)
                            tcs_tax = state_journal and state_journal.qshop_tcs_tax_id
                            if line.tcs_tax_id:
                                tcs_tax = line.tcs_tax_id
                            if tcs_tax:
                                taxes = tcs_tax.with_context(round=True).compute_all(line.total_pay)
                                for tax in taxes['taxes']:
                                    move_line_5 = {'account_id': tax['account_id'] or False,
                                                   'debit': 0.0,
                                                   'credit': tax['amount'],
                                                   'analytic_distribution': {
                                                                                rec.region_id.analytic_account_id.id: 100} or False,
                                                   'name': tax['name'] + ' ' + 'Merchant Amount-TCS',
                                                   'partner_id': line.customer_id.id or False,
                                                   'tag_ids': tax['tag_ids'] or [],
                                                   'service_type_id': qshop_service_type.id,
                                                   }
                                    move_line_list.append((0, 0, move_line_5))

                        if line.final_pay >= 1:
                            move_line_1 = {'account_id': accounts_data.qshop_credit_transfer_account_id.id or False,
                                           'debit': 0.0,
                                           'name': 'Qwqer Shop Merchant Amount',
                        ...
                        for trans_line in line.line_ids:
                            trans_line.qshop_merchant_payout_id = rec.id
    ...
    def get_start_and_end(self, date):
        if date:
            ...
            tz_name = self._context.get('tz', False) or self.env.user.tz
            if not tz_name:
    ...
    def get_line_status(self, transfer_id, line_id):
        ...
        def reset_line_entries(line):
            ...
            if line.re_initiated != "yes":
                ...
                if line.line_ids:
                    line.line_ids.write({'qshop_merchant_payout_id': False})
        ...
        for rec in self:
            ...
            for line in lines:
                invoice_line_vals = {
                    ...
                    'tax_ids': [(6, 0, line.customer_id.qshop_invoice_tax_ids.ids)],
                    'service_type_id': qshop_service_type.id,
                }
                ...
                line_invoice = {'move_type': 'out_invoice',
                                ...
                                'journal_id': state_journal.qshop_journal_id.id,
                                'ref': rec.rec_name,
                                'service_type_id': qshop_service_type.id,

...
export class FieldCityGeoLocationAutocomplete extends Component {
    ...
    setup() {
        ...
        this.inputFiledName = inputFiledName;
         ...
         // Fetch track_trip_uid value
        this.trackTripUid = this.props.record.data.track_trip_uid;
        ...
        onMounted(() => {
            if (typeof google !== 'undefined' && google.maps && google.maps.places) {
               ...
               // Apply readonly condition on mount
            this.applyReadonlyCondition();
        });
    ...
    applyReadonlyCondition() {
        // Dynamically make the input readonly based on track_trip_uid
        if (this.input.el) {
            const isReadonly = this.isReadonly();
            this.input.el.readOnly = isReadonly; // Apply readonly to the input field
        }
    }

    isReadonly() {
        // Return true if track_trip_uid is not false or null
        return Boolean(this.trackTripUid);
    }

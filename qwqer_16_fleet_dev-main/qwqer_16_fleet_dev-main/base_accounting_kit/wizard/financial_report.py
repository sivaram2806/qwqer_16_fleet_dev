...
class FinancialReport(models.TransientModel):
    ...
    def view_report_xl(self):
        ...
        return self.env.ref(
            'base_accounting_kit.financial_report_xl').report_action(self,
                                                                     data)
...
class AccountingExcelReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        worksheet = workbook.add_worksheet('Accounting Report.xlsx')
        ...
        worksheet.merge_range('A1:E2', company_name, head)
        worksheet.merge_range('A3:E4', company_addr, head2)
        worksheet.merge_range('A5:E6', company_addr1, head2)

        row = 7
        column = 0

        worksheet.merge_range('A7:E7', data['form']['account_report_id'][1], head)
        row += 2
        ...
        if data['form']['date_to']:
            to_date = fields.Date.from_string(data['form']['date_to']).strftime('%d/%m/%Y')
        else:
            to_date = ''

        worksheet.write(row, column, 'Target Moves:', cell_text_format)
        worksheet.write(row, column + 2, 'Date from:', cell_text_format)
        worksheet.write(row, column + 3, from_date, cell_text_format)
        row += 1
        if data['form']['target_move'] == 'all':
            worksheet.write(row, column, 'All Entries', cell_text_format)
        if data['form']['target_move'] == 'posted':
            worksheet.write(row, column, 'All Posted Entries', cell_text_format)

        worksheet.write(row, column + 2, 'Date to:', cell_text_format)
        worksheet.write(row, column + 3, to_date, cell_text_format)

        record = self.env['financial.report'].browse(data['active_id'])
        report_lines = record.get_account_lines(data['form'])
        ...
        if data['form']['debit_credit'] == 1:
            worksheet.write(row, column, 'Name', cell_text_format)
            worksheet.write(row, column + 2, 'Debit', cell_text_format)
            worksheet.write(row, column + 3, 'Credit', cell_text_format)
            worksheet.write(row, column + 4, 'Balance', cell_text_format)
            ...
            for res in report_lines:
                if res['level'] - 1 == 1:
                    ...
                    row += 1
                x = '  ' * (int(res['level'] - 1) * 4) + res['name']
                worksheet.write(row + 1, column, x, format3)
                worksheet.write(row + 1, column + 2, res['debit'], format3)
                worksheet.write(row + 1, column + 3, res['credit'], format3)
                worksheet.write(row + 1, column + 4, res['balance'], format3)

        if not data['form']['enable_filter'] and not data['form']['debit_credit']:
            worksheet.write(row, column, 'Name', cell_text_format)
            worksheet.write(row, column + 2, 'Balance', cell_text_format)
            ...
            for res in report_lines:
                ...
                x = '  ' * (int(res['level'] - 1) * 4) + res['name']
                worksheet.write(row + 1, column, x, format3)
                worksheet.write(row + 1, column + 2, res['balance'], format3)

        if data['form']['enable_filter'] == 1 and not data['form']['debit_credit']:
            worksheet.write(row, column, 'Name', cell_text_format)
            worksheet.write(row, column + 2, 'Balance', cell_text_format)
            worksheet.write(row, column + 3, 'Comp', cell_text_format)
            ...
            for res in report_lines:
                ...
                worksheet.write(row + 1, column, x, format3)
                worksheet.write(row + 1, column + 2, res['balance'], format3)
                worksheet.write(row + 1, column + 3, res['balance_cmp'], format3)

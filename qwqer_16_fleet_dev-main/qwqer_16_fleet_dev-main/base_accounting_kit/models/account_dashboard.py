    ...
    def month_income(self):
        ...
        self._cr.execute('''
            ...
            FROM 
                account_move
            JOIN 
                account_move_line ON account_move_line.move_id = account_move.id
            JOIN 
                account_account ON account_move_line.account_id = account_account.id
            WHERE 
                account_move.move_type = 'entry' 
                AND account_move.state = 'posted' 
                AND account_account.internal_group = 'income'
                AND TO_CHAR(account_move.date, 'YYYY-MM') = TO_CHAR(NOW(), 'YYYY-MM');
        ''')
        result = self._cr.fetchone() or (0.0, 0.0)
        return (result[0] or 0.1, result[1] or 0.1)

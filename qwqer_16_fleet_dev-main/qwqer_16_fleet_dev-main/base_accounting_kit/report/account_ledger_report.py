...
class AccountLedger(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if account_filter:
            if account_filter == 'receivable':
                all_accounts_objs = self.env['account.account'].search([('account_type','=','asset_receivable')],order="code asc")

            elif account_filter == 'payable':
                all_accounts_objs = self.env['account.account'].search([('account_type','=','liability_payable')],order="code asc")

            else:
                all_accounts_objs = self.env['account.account'].search([('account_type','in',('liability_payable','asset_receivable'))],order="code asc")
        ...
        for account_id in account_ids:
            ...
            if summary_format:
                ...
                # mvline_list = self.env['account.move.line'].search([('date','>=',from_date),
                                                                # ('date','<=',to_date),
                                                                # ('account_id','=',account_id),
                                                                # ('parent_state','=','posted')
                                                                # ])
                                                                #
                # mv_domain = [('date','>=',from_date),
                          # ('date','<=',to_date),
                          # ('account_id','=',account_id),
                        # ('parent_state','=','posted')]
                ...
                # sql = """
                # SELECT id  FROM %(from)s WHERE %(where)s 
                # """ % {'from': from_clause, 'where': where_clause}
                # self.env.cr.execute(sql, where_clause_params)
                # mvline_list = list(itertools.chain(*self.env.cr.fetchall()))
                
                # print("=============================== ",mvline_list,len(mvline_list),account_id) 
                ...
                journal_list = []
                # if mvline_list:
                # debit_list = self.env['account.move.line'].search([('date','>=',from_date),
                                                            # ('date','<=',to_date),
                                                            # ('account_id','=',account_id),
                                                            # ('parent_state','=','posted'),
                                                            # ('debit','>',0)
                ...
                from_clause, where_clause, where_clause_params = query.get_sql()
                sql = """
                SELECT distinct(move_id)  FROM %(from)s WHERE %(where)s 
                """ % {'from': from_clause, 'where': where_clause}
                self.env.cr.execute(sql, where_clause_params)
                debit_list = list(itertools.chain(*self.env.cr.fetchall()))                                            
                
                
                
                # credit_list = self.env['account.move.line'].search([('date','>=',from_date),
                                                            # ('date','<=',to_date),
                                                            # ('account_id','=',account_id),
                                                            # ('parent_state','=','posted'),
                                                            # ('credit','>',0)
                ...
                credit_domain = [('date','>=',from_date),
                      ('date','<=',to_date),
                      ('account_id','=',account_id),
                      ('credit','>',0.00),
                    ('parent_state','=','posted')]
                query = self.env['account.move.line']._where_calc(credit_domain)
                from_clause, where_clause, where_clause_params = query.get_sql()
                sql = """
                SELECT distinct(move_id)  FROM %(from)s WHERE %(where)s 
                """ % {'from': from_clause, 'where': where_clause}
                self.env.cr.execute(sql, where_clause_params)
                credit_list = list(itertools.chain(*self.env.cr.fetchall()))
                
                
                # print("mvline_debt_list ========= ",len(mvline_debt_list))
                # for line in mvline_list:
                    # #Cash In Flow
                    # if line.debit:
                        # debit_list.append(line)
                    # #Cash Out Flow                            
                    # if line.credit:
                        # credit_list.append(line)
                # print("debit_list ---------- ",len(debit_list),debit_list)        
                if debit_list:
                    
                    domain = [('move_id','in',debit_list),
                              ('account_id','!=',account_id),
                              ('parent_state','=','posted')]
                    query = self.env['account.move.line']._where_calc(domain)
                    from_clause, where_clause, where_clause_params = query.get_sql()
                    ...
                    """ % {'from': from_clause, 'where': where_clause}
                    self.env.cr.execute(sql, where_clause_params)
                    journal_list_rec = list(itertools.chain(*self.env.cr.fetchall()))
                            
                    # print("========================== ",journal_list_rec)
                    if not debit_journal_list:
                        debit_journal_list = journal_list_rec
                    else:
                        debit_journal_list = debit_journal_list + journal_list_rec
                    # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ",debit_journal_list)
                    
                    # for debit_list_elem in debit_list:
                        # if debit_list_elem.move_id and debit_list_elem.move_id.line_ids:
                            # domain = [('move_id','=',debit_list_elem.move_id.id),
                                      # ('account_id','!=',account_id),
                                      # ('parent_state','=','posted')]
                            # query = self.env['account.move.line']._where_calc(domain)
                            # from_clause, where_clause, where_clause_params = query.get_sql()
                            ...
                            #
                            #
                            # # journal_list_rec = self.env['account.move.line'].search([
                                                            # # ('move_id','=',debit_list_elem.move_id.id),
                                                            # # ('account_id','!=',debit_list_elem.account_id.id),
                                                            # # ('parent_state','=','posted')
                                                            # # ])
                            # print("========================== ",journal_list_rec)
                            # if not debit_journal_list:
                            ...
                            # else:
                                # debit_journal_list = debit_journal_list + journal_list_rec
                                # # debit_journal_list = list(itertools.chain(*journal_list_rec))
                            # # debit_journal_list.append(journal_list_rec.pop())                                
                            # # for journal_item in debit_list_elem.move_id.line_ids:
                                # # if journal_item.credit:
                                # #
                                    # # debit_journal_list.append(journal_item.id)
                                # # if journal_item.debit and not journal_item.account_id.id == debit_list_elem.account_id.id:
                                    # # debit_journal_list.append(journal_item.id)
                            # # debit_journal_list = list(dict.fromkeys(debit_journal_list))
                            #
                # print("debit_journal_list --------------------- ",debit_journal_list)                    
                if credit_list:
                    domain_credit = [('move_id','in',credit_list),
                              ('account_id','!=',account_id),
                              ('parent_state','=','posted')]
                    query = self.env['account.move.line']._where_calc(domain_credit)
                    from_clause, where_clause, where_clause_params = query.get_sql()
                    sql = """
                    SELECT id  FROM %(from)s WHERE %(where)s 
                    """ % {'from': from_clause, 'where': where_clause}
                    self.env.cr.execute(sql, where_clause_params)
                    cred_journal_list_rec = list(itertools.chain(*self.env.cr.fetchall()))
                            
                    # print("========================== ",cred_journal_list_rec)
                    if not debit_journal_list:
                        credit_journal_list = cred_journal_list_rec
                    else:
                        credit_journal_list = debit_journal_list + cred_journal_list_rec
                    
                    
                    # for credit_list_elem in credit_list:
                        # if credit_list_elem.move_id and credit_list_elem.move_id.line_ids:
                            # for journl_item in credit_list_elem.move_id.line_ids:
                                # if journl_item.debit:
                                    # credit_journal_list.append(journl_item.id)
                                # if journl_item.credit and not journl_item.account_id.id == credit_list_elem.account_id.id:
                                    # credit_journal_list.append(journl_item.id)
                                # credit_journal_list = list(dict.fromkeys(credit_journal_list))
                
                journal_list.extend(debit_journal_list)
                journal_list.extend(credit_journal_list)
                
                # print("journal list ######################### ",journal_list,len(journal_list),len(credit_journal_list),len(debit_journal_list))
                journal_list = list(dict.fromkeys(journal_list))
                if journal_list:
                    journal_string = ""
                    if journal_list:
                        if len(journal_list) == 1:
                            journal_string = "= "+str(journal_list[0])
                        else:
                            journal_string =  "in "+str(tuple(journal_list))
                    
                    
                    
                    
                    
                    where_condition = """(aml.id {0})""".format(journal_string)
                    ...
                    cr.execute("""
                                Select
                                    ...
                                    aml.name as label,
                                    Case when 
                                        aa.account_type In ('payable','receivable')
                                        Then
                                            Case when
                                                am.name is null 
                                            Then 
                    ...
                    if trnacation_details or (opening_balance and include_opening_bal) or not include_movement_filter:
                        ...
                        if trnacation_details:
                            ...
                            worksheet.write(row, 3, 'Account Name', sub_heading_center)
                            worksheet.write(row, 4, 'Partner Name', sub_heading_center)
                            worksheet.write(row, 5, 'Debit', sub_heading_right)
                            worksheet.write(row, 6, 'Credit', sub_heading_right)
                            worksheet.write(row, 7, 'Balance', sub_heading_right)
                            ...
                            if opening_balance and include_opening_bal:
                                ...
                                worksheet.write(row, 7, opening_balance[0], op_bal_heading_right)
            #                     account_total += opening_balance[0]
                            ...
                            for items in trnacation_details:
                                ...
                                worksheet.write(row, 3, items.get('account_name'), column_details_left)
                                worksheet.write(row, 4, items.get('partner'), column_details_left)
                                worksheet.write(row, 5, items.get('credit'), column_numbers_right)
                                worksheet.write(row, 6, items.get('debit'), column_numbers_right)
                                balance_log -= (items.get('debit') - items.get('credit'))
                                worksheet.write(row, 7, balance_log, column_numbers_right)
        #                         account_total += items.get('balance')
            ...
            #                     account_total += opening_balance[0]
                            ...
                            worksheet.write(row, 4, '', column_details_left)
    ...
    #                         account_total += items.get('balance')
                        ...
                        if account_credit_total > 0.0 or account_debit_total > 0.0:
                            row += 1
                            worksheet.write(row, 4, 'Total', sub_heading_right)
                            worksheet.write(row, 6, account_debit_total, sub_total_right)
                            worksheet.write(row, 5, account_credit_total, sub_total_right)
                            worksheet.write(row, 7, balance_log, sub_total_right)
                        elif not include_movement_filter:
                            row += 1
                            worksheet.write(row, 4, 'Total', sub_heading_right)
                            worksheet.write(row, 5, 0.00, sub_total_right)
                            worksheet.write(row, 6, 0.00, sub_total_right)
        ...
        if summary_format and summary_added:
            ...
            worksheet.write(row, 6, summary_grand_debit, total_heading_right)

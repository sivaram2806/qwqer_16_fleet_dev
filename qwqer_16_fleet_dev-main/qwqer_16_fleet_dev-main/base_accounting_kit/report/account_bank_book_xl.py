...
class BankBookExcelReport(models.AbstractModel):
    _name = "report.base_accounting_kit.report_bank_book_xlsx"
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby,
        ...
        print("================================ ",filters," ********* ",self.env.context.get('date_from'),where_clause)
        # sql = ('''SELECT l.id AS lid, l.account_id \
        # AS account_id, l.date AS ldate, \
         # l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname\
         # FROM account_move_line l\
         # WHERE l.account_id IN %s ''' + filters + ''' GROUP BY \
         # l.id, l.account_id, l.date,  l.currency_id, \
         # l.amount_currency, l.ref, l.name \
         # ORDER BY ''' + sql_sort)
        ...
        for account_rec in accounts:   
            print(" account_rec.id --- ",account_rec.id)  
            mvline_list = self.env['account.move.line'].search([('date','>=',self.env.context.get('date_from')),
                                                                ('date','<=',self.env.context.get('date_to')),
                                                                ('account_id','=',account_rec.id),
                                                                ('parent_state','=',self.env.context.get('state'))
                                                                ])    
            
            print("=============================== ",mvline_list,len(mvline_list),account_rec.name) 
            ...
            if mvline_list:
                ...
                if debit_list:
                    for debit_list_elem in debit_list:
                        print("=============================== ",debit_list_elem.move_id.line_ids)
            ...
            if journal_list:
                ...
                AS account_id, l.date AS ldate, j.code AS lcode,\
                 l.currency_id,acc.name AS account_name, l.amount_currency, l.ref AS lref, l.name AS lname,\
                  COALESCE(l.debit,0) AS credit, \
                  COALESCE(l.credit,0) AS debit, \
                  COALESCE(SUM(l.credit),0) - COALESCE(SUM(l.debit), 0) AS balance,\
                        ...
                        WHERE l.id '''+journal_string+''' ''' + filters + ''' GROUP BY \
                        l.id, l.account_id, l.date, j.code, l.currency_id, \
                        l.amount_currency, l.ref, l.name, m.name, \
                        c.symbol, p.name,acc.name ORDER BY ''' + sql_sort)
                ...
                for row in cr.dictfetchall():
                    balance = 0
                    print("row['account_id'] ------------------------ ",row)
                    for line in move_lines.get(account_rec.id):
                        balance += line['debit'] - line['credit']
                    row['balance'] += balance
                    move_lines[account_rec.id].append(row)
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        worksheet = workbook.add_worksheet('Bank Book.xlsx')
        ...
        worksheet.merge_range('B7:E7', 'Bank Book Report', head)
        row += 2
        ...
        worksheet.write(row, column + 2, 'Account', cell_text_format)
        worksheet.write(row, column + 3, 'Partner', cell_text_format)
        worksheet.write(row, column + 4, 'Ref', cell_text_format)
        worksheet.write(row, column + 5, 'Move', cell_text_format)
        worksheet.write(row, column + 6, 'Entry Label', cell_text_format)
        worksheet.write(row, column + 7, 'Debit', cell_text_format)
        worksheet.write(row, column + 8, 'Credit', cell_text_format)
        ...
        for account in accounts_res:
            ...
            worksheet.write(row + 1, column + 6, '', format3)
            worksheet.write(row + 1, column + 7, account['debit'], format3)
            worksheet.write(row + 1, column + 8, account['credit'], format3)
            worksheet.write(row + 1, column + 9, account['balance'], format3)
            row += 1
            for line in account['move_lines']:
                if line['ldate']:
                    ldate = fields.Date.from_string(line['ldate']).strftime('%d/%m/%Y')
                    worksheet.write(row + 1, column, ldate, format3)
                worksheet.write(row + 1, column + 1, line['lcode'], format3)
                worksheet.write(row + 1, column + 2, line['account_name'], format3)
                worksheet.write(row + 1, column + 3, line['partner_name'], format3)
                worksheet.write(row + 1, column + 4, line['lref'], format3)
                worksheet.write(row + 1, column + 5, line['move_name'], format3)
                worksheet.write(row + 1, column + 6, line['lname'], format3)
                # if line['lname'] == 'Initial Balance':
                worksheet.write(row + 1, column + 7, line['debit'], format3)
                worksheet.write(row + 1, column + 8, line['credit'], format3)
                # else:
                    # worksheet.write(row + 1, column + 7, line['credit'], format3)
                    # worksheet.write(row + 1, column + 8, line['debit'], format3)
                worksheet.write(row + 1, column + 9, line['balance'], format3)
                row += 1

...
class PartnerLedger(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        partner_ids = (record.partner_ids and record.partner_ids.ids) or (
                    all_partner_objs and all_partner_objs.ids) or []
            ...
            if summary_format:
                ...
                if not account_filter and account_ids:
                    ...
                    if account_filter == 'receivable':
                        summary_where_condition += """ AND aa.account_type = 'asset_receivable'"""

                    elif account_filter == 'payable':
                        summary_where_condition += """ AND aa.account_type = 'liability_payable'"""

                    else:
                        summary_where_condition += """ AND aa.account_type In ('liability_payable','asset_receivable')"""
                ...
                total_debit = dict_val and dict_val.get('total_debit') is not None and [
                    dict_val.get('total_debit')] or False
                total_credit = dict_val and dict_val.get('total_credit') is not None and [
                    dict_val.get('total_credit')] or False
                ...
                if from_date:
                    ...
                    if account_ids and not account_filter:
                        ...
                        if account_filter == 'receivable':
                            ...
                            opening_where_condition += """ AND aa.account_type In ('liability_payable','asset_receivable')"""
                    ...
                    dict_val = cr.dictfetchall()
                    dict_val = dict_val and dict_val[0] or False
                    opening_balance = dict_val and dict_val.get('balance') is not None and [
                        dict_val.get('balance')] or False
                ...
                if account_ids and not account_filter:
                    ...
                    if account_filter == 'receivable':
                        where_condition += """ AND aa.account_type = 'asset_receivable'"""

                    elif account_filter == 'payable':
                        where_condition += """ AND aa.account_type = 'liability_payable'"""

                    else:
                        where_condition += """ AND aa.account_type In ('liability_payable','asset_receivable')"""

...
class HdfcSalaryXls(models.AbstractModel):
    _name = 'report.payment_request.hdfc_export_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, active_model):
        ...
        for line in active_model:
            ...
            col = 0
            txn_type = 'N'
            if line.partner_id.bank_name:
                name_list =line.partner_id.bank_name.split()
            ...
            col += 1
            worksheet.write(row, col, line.partner_id.account_no or '', design_formats['normal_format_central'])
            col += 1
            worksheet.write(row, col, round(line.amount_residual,2), design_formats['amount_format'])
            col += 1
            if len(line.partner_id.name) <= 15:
                worksheet.write(row, col, line.partner_id.name or '', design_formats['heading_format_1'])
            else:
            ...
            if txn_type == 'I':
                ...
                if len(line.partner_id.name) <= 15:
                    worksheet.write(row, col,line.partner_id.name or '', design_formats['normal_format_central'])
                else:
                    name = line.partner_id.name
                    names = name[0:15]
                    worksheet.write(row, col,names or '', design_formats['normal_format_central'])
            col += 1
            worksheet.write(row, col, line.name or '', design_formats['heading_format_1'])
            col += 1
            worksheet.write(row, col, '', design_formats['heading_format_1'])
            col +=1
            worksheet.write(row, col, '', design_formats['heading_format_1'])
            ...
            worksheet.write(row, col, '', design_formats['heading_format_1'])
            col +=1
            # date = datetime.strftime(active_model.date_end, "%d/%m/%Y")
            date = datetime.now().strftime('%d/%m/%Y')
            worksheet.write(row, col, date or '', design_formats['normal_format_central'])
            col += 1
            worksheet.write(row, col, '', design_formats['heading_format_1'])
            col +=1
            worksheet.write(row, col, line.partner_id.ifsc_code or '', design_formats['heading_format_1'])
            col += 1
            worksheet.write(row, col, line.partner_id.bank_name or '', design_formats['heading_format_1'])
            col += 1
            worksheet.write(row, col, '', design_formats['heading_format_1'])
            col += 1
            worksheet.write(row, col, line.partner_id.email or 'accounts@qwytech.com', design_formats['heading_format_1'])
            row += 1

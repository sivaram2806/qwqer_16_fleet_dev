...
class ReInitiateWiz(models.TransientModel):
    _name = "re.init.payout.wiz"
    _description = "ReInitiated Wizard"
    
    payout_ids = fields.Many2many('delivery.merchant.payout.lines',string="ReInitiated")

    @api.model
    def default_get(self, fields):
        res = super(ReInitiateWiz, self).default_get(fields)
        if self._context.get('active_model') == 'delivery.merchant.payout' and self._context.get('active_id', False):
            rec_id = self.env['delivery.merchant.payout'].browse(self._context.get("active_id"))
    ...
    def action_repayment(self):
        for rec in self:
            if self._context.get('active_model') == 'delivery.merchant.payout' and self._context.get('active_id', False):
                data = self.env['delivery.merchant.payout'].browse(self._context.get("active_id"))
                if data and rec.payout_ids:
                    ...
                    if list:
                        ...
                        payout_id = self.env['delivery.merchant.payout'].sudo().create(
                        ...
                        for payout_line in payout_id.line_ids:
                            for trans_line in payout_line.line_ids:
                                trans_line.merchant_payout_id = payout_id.id
                        return {
                            'name': _('Delivery Merchant Payouts'),
                            'view_type': 'form',
                            'view_mode': 'form',
                            'res_model': 'delivery.merchant.payout',
                            'view_id': False,

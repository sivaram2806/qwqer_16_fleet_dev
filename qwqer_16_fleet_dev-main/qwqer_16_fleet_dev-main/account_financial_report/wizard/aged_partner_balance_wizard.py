...
class AgedPartnerBalanceWizard(models.TransientModel):
    ...
    partner_ids = fields.Many2many("res.partner", "partner_aged_report_rel", string="Filter partners")
    ...
    customer_type = fields.Selection([('b2c', 'B2C'),
                                         ('b2b', 'B2B'),
                                         ], string="Customer Type")
    state_ids = fields.Many2many('res.country.state', 'rel_res_country_state',
                                'country_state_id', 'state_id', string='State', domain=get_states)
    region_ids = fields.Many2many('sales.region', 'rel_region',
                                  'country_region_id', 'region_id', string='Region')
    # add fleet
    service_type_id = fields.Many2one('partner.service.type', "Service Type")
    ...
    def onchange_state_ids(self):
        new_list = []
        all_regions_list = []
        all_regions = self.env['sales.region'].search([])
        if all_regions:
            for each_region in all_regions:
                all_regions_list.append(each_region.id)
        if self.state_ids:
            regions = self.env['sales.region'].search([('state_id', 'in', self.state_ids.ids)])
            if regions:
                for region in regions:
                    new_list.append(region.id)
                return {'domain': {'region_ids': [('id', 'in', new_list)]}}
            else:
                return {'domain': {'region_ids': [('id', 'in', new_list)]}}
        else:
            return {'domain': {'region_ids': [('id', 'in', all_regions_list)]}}

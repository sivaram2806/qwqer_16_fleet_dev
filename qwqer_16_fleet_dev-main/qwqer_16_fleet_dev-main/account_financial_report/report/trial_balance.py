...
ACCOUNT_TYPE = {"asset_receivable": "Receivable",
...
class TrialBalanceReport(models.AbstractModel):
    ...
    def _get_internal_type_groups_data(self, accounts_data, total_amount=None, foreign_currency=None):
        internal_type_group = {}
        for account in accounts_data.values():
            accounts_id = account.get('id')
            account_rec = self.env["account.account"].browse(accounts_id)
            if account_rec.account_type:
                if account_rec.account_type not in internal_type_group.keys():
                    internal_type_group.update({account_rec.account_type: [account]})
                else:
                    internal_type_group[account_rec.account_type].append(account)
        internal_type_group_tot = {}
        for internal_type in internal_type_group.keys():
            internal_type_group_tot.update(
                {
                    internal_type: {
                        "id": internal_type,
                        "name": ACCOUNT_TYPE[internal_type],
                        "type": "internal_type",
                        'level': 1,
                        "initial_balance": 0.0,
                        "credit": 0.0,
        ...
        #         groups_data[group.id]["ending_currency_balance"] = 0.0
        group_data = []
        for group_id in internal_type_group.keys():
            for account in internal_type_group[group_id]:
                internal_type_group_tot[group_id]["initial_balance"] += total_amount[account.get('id')][
                    "initial_balance"
                ]
                internal_type_group_tot[group_id]["debit"] += total_amount[account.get('id')]["debit"]
                internal_type_group_tot[group_id]["credit"] += total_amount[account.get('id')]["credit"]
                internal_type_group_tot[group_id]["balance"] += total_amount[account.get('id')]["balance"]
                internal_type_group_tot[group_id]["ending_balance"] += total_amount[account.get('id')][
                    "ending_balance"
                ]
        for rec in internal_type_group_tot.values():
            group_data.append(rec)
            for item in internal_type_group.get(rec.get('id')):
    ...
    def _get_report_values(self, docids, data):
        ...
        if not show_partner_details:
            for account_id in accounts_data.keys():
                if total_amount[account_id]["initial_balance"] > 0:
                    total_data["initial_balance_debit"] += total_amount[account_id]["initial_balance"]
                else:
                    total_data["initial_balance_credit"] += total_amount[account_id]["initial_balance"]
                total_data["debit"] += total_amount[account_id]["debit"]
                total_data["credit"] += total_amount[account_id]["credit"]
                if total_amount[account_id]["balance"] > 0:
                    total_data["balance_debit"] += total_amount[account_id]["balance"]
                else:
                    total_data["balance_credit"] += total_amount[account_id]["balance"]
                if total_amount[account_id]["ending_balance"] > 0:
                    total_data["ending_balance_debit"] += total_amount[account_id]["ending_balance"]
                else:
                    total_data["ending_balance_credit"] += total_amount[account_id]["ending_balance"]
                total_data_xlsx["initial_balance"] += total_amount[account_id]["initial_balance"]
                total_data_xlsx["debit"] += total_amount[account_id]["debit"]
                total_data_xlsx["credit"] += total_amount[account_id]["credit"]
                total_data_xlsx["balance"] += total_amount[account_id]["balance"]
                total_data_xlsx["ending_balance"] += total_amount[account_id]["ending_balance"]
            ...
            if hierarchy_on == "internal_type":
                group_data = self._get_internal_type_groups_data(
                    accounts_data, total_amount, foreign_currency
                )
                trial_balance = group_data
        ...
        return {
            ...
            "total_data_xlsx": total_data_xlsx,
